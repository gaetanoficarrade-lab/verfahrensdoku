...
const ClientRegister = () => {
  ...
  useEffect(() => {
    if (!token) {
      setTokenValid(false);
      setErrorMessage('Kein Einladungstoken angegeben.');
      return;
    }
    const validate = async () => {
      ...
      if (!anyToken) {
        setTokenValid(false);
        setErrorMessage('Dieser Einladungslink ist ungültig. Bitte kontaktieren Sie Ihren Berater für einen neuen Link.');
        return;
      }

      if (anyToken.used_by) {
        setTokenValid(false);
        setErrorMessage('Dieser Einladungslink wurde bereits verwendet. Falls Sie bereits ein Konto haben, melden Sie sich bitte an.');
        return;
      }

      if (!anyToken.is_active) {
        setTokenValid(false);
        setErrorMessage('Dieser Einladungslink wurde widerrufen. Bitte kontaktieren Sie Ihren Berater für einen neuen Link.');
        return;
      }

      if (new Date(anyToken.expires_at) < new Date()) {
        setTokenValid(false);
        setErrorMessage('Dieser Einladungslink ist abgelaufen. Bitte kontaktieren Sie Ihren Berater für einen neuen Link.');
        return;
      }
      ...
      >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Erstellen Sie Ihr Konto, um auf Ihre Verfahrensdokumentation zuzugreifen.
            </CardDescription>
          </CardHeader>
          <CardContent>
